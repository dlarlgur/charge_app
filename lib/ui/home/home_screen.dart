        ...
        floatingActionButton: bottomIndex == 0
            ? Builder(builder: (_) {
                ...
                return HomeQuickFab(items: [
                  // 응원 진입점 — 스트립을 지나쳤어도 여기서 다시 만난다.
                  // 첫 항목 + 골드 강조 (핸드오프 4).
                  // NEW 배지는 안 단다 — 끌 조건(기간·최초 노출 이후)이 없으면 영구히
                  // 붙어 있게 되고, 그 시점부터는 사실이 아닌 배지가 된다.
                  QuickMenuItem(
                    icon: Icons.volunteer_activism_rounded,
                    label: '개발자 응원하기',
                    color: AppColors.supportAccent,
                    highlight: true,
                    onTap: _openCheerScreen,
                  ),
    ...
    return RefreshIndicator(
      ...
      child: CustomScrollView(
        ...
        slivers: [
          ...
          // 응원 스트립 — 요약 카드 바로 아래, 리스트 첫 카드 위 (핸드오프 3-3).
          const SliverToBoxAdapter(child: CheerStrip()),
          // 리스트
          stationsAsync.when(
            loading: () => SliverList(
